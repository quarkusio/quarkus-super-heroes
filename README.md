...
## Deploying with Helm
Pre-built Helm charts for all of the applications in the system can be found in the [`deploy/helm`](deploy/helm) directory. There are umbrella charts for each deployment target and version combination.

> [!NOTE]
> Helm charts are generated by the [Quarkus Helm extension](https://docs.quarkiverse.io/quarkus-helm/dev/index.html). Each service has per-deployment-target charts with separate values files for JVM and native variants. Users must specify a values file at install time.
...
Pick one of the umbrella charts from the table below. Each umbrella chart includes all services plus monitoring as dependencies.

| Description | OpenShift | Minikube | Kubernetes | Knative |
|-------------|-----------|----------|------------|---------|
| JVM Java 21 | [`java21-openshift`](deploy/helm/java21-openshift) | [`java21-minikube`](deploy/helm/java21-minikube) | [`java21-kubernetes`](deploy/helm/java21-kubernetes) | [`java21-knative`](deploy/helm/java21-knative) |
| Native      | [`native-openshift`](deploy/helm/native-openshift) | [`native-minikube`](deploy/helm/native-minikube) | [`native-kubernetes`](deploy/helm/native-kubernetes) | [`native-knative`](deploy/helm/native-knative) |

To deploy the full system (e.g. JVM Java 21 on Kubernetes):
```shell
helm dependency update deploy/helm/java21-kubernetes
helm install super-heroes deploy/helm/java21-kubernetes
```
