...
This workflow runs on every push to `main` and on all pull requests. It can also be triggered manually. It builds and tests all 7 services using Java 21:
...
- **Build JVM container images** -- Builds JVM-based container images for each service on both `amd64` and `arm64` architectures using Java 21.
...
5. Both JVM (Java 21) and native variants are generated.
