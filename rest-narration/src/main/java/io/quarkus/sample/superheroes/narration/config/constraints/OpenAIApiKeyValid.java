...
@Constraint(validatedBy = OpenAiApiKeyValidator.class)
@Target({ ElementType.TYPE, ElementType.ANNOTATION_TYPE, ElementType.METHOD, ElementType.TYPE_USE })
@Retention(RetentionPolicy.RUNTIME)
@Documented
public @interface OpenAIApiKeyValid {
  String message() default "OpenAI configuration is enabled (narration.open-ai.enabled = true) but no api key (narration.open-ai.api-key) is specified. Please configure an api key.";
  Class<?>[] groups() default {};
  Class<? extends Payload>[] payload() default {};

  class OpenAiApiKeyValidator implements ConstraintValidator<OpenAIApiKeyValid, OpenAI> {
    @Override
    public boolean isValid(OpenAI openAI, ConstraintValidatorContext context) {
      return (openAI.enabled() && openAI.apiKey().isEmpty()) ? false : true;
    }
  }
}
