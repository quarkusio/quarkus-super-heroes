...
do
  ...
  do
    ...
    do
      ...
      set -x
      cat $proj/deploy/${kind}java${version}-kubernetes.yml >> $kubernetes_result
      cat $proj/deploy/${kind}java${version}-minikube.yml >> $minikube_result
      cat $proj/deploy/${kind}java${version}-openshift.yml >> $openshift_result
      set +x
    done
